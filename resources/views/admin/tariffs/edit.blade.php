    ...
    <div class="bg-white shadow-sm sm:rounded-lg p-6">
        <form method="POST" action="{{ route('admin.tariffs.update', $tariff) }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $tariff->name) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" name="time_start" value="{{ old('time_start', \Illuminate\Support\Str::of($tariff->time_start)->limit(5, '')) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                ...
                <div>
                    ...
                    <input type="time" name="time_end" value="{{ old('time_end', \Illuminate\Support\Str::of($tariff->time_end)->limit(5, '')) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
            ...
            <div>
                ...
                <input type="number" step="0.01" min="0" name="price_per_hour" value="{{ old('price_per_hour', $tariff->price_per_hour) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                @error('price_per_hour')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
            </div>

            <div class="flex items-center">
                <input type="hidden" name="is_active" value="0">
                <input id="is_active" type="checkbox" name="is_active" value="1" @checked(old('is_active', $tariff->is_active)) class="h-4 w-4 text-indigo-600 border-gray-300 rounded">
